...
class BudgetsListagePending extends StatefulWidget {
  BudgetsListagePending({super.key});

  @override
  _BudgetsListagePendingState createState() => _BudgetsListagePendingState();
}

class _BudgetsListagePendingState extends State<BudgetsListagePending> {
        ...
        .then((budgets) {
          setState(() {
            _allBudgets = budgets.where((budget) {
              return budget['status']?.toString().toLowerCase() == 'pendente';
            }).toList();
            _filteredBudgets = _allBudgets;
          });
        })
        .catchError((error) {
